    ...
    with codecs.open(path, 'r', 'utf-8') as file:
        for line in file:
            if line.strip() == "":
                ...
                poss.append(pos)
                labels.append(label)
    return word_sentences, pos_sentences, label_sentences


def create_data_2_train(train_path, dev_path, test_path, char_embedd_dim):
    word_sentences_train, pos_sentences_train, label_sentences_train = load_conll_data(train_path)
    word_sentences_dev, pos_sentences_dev, label_sentences_dev = load_conll_data(dev_path)
    word_sentences_test, pos_sentences_test, label_sentences_test = load_conll_data(test_path)
    ...
    label_sentences_id_train, alphabet_label = utils.map_string_2_id_open(label_sentences_train, 'chunk')
...
def save_config(config_file):
    with codecs.open(config_file, 'w', 'utf-8') as f:
        ...
        f.write('embedd_dim_concat' + '\t' + str(embedd_dim_concat) + '\n')
